...
export function installDirectSshRetryStatus(session: ConnectPanePtySession): void {
  ...
  // Why a generation rather than a bare timestamp: input sent to a previous PTY
  // must not make a replacement shell look user-interacted, and acknowledged
  // writes settle asynchronously, so a fast PTY exit can beat the ack callback.
